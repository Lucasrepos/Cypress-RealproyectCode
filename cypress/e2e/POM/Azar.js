...
function getApellido() {
    ...
    let random = Math.floor(Math.random() * lastname.length);
    return lastname[random];



}
function Nombrepuntual() {

    var specialname = ['MENDOZA Justin'];
...
const generaDNI = (longitud) => {
    let getDni = "";
    for (let x = 0; x < longitud; x++) {
        let aleatorio = Math.floor(Math.random() * numeros.length);
        getDni += numeros.charAt(aleatorio);
    }
    return getDni;
};

export { getNombre, getApellido, Nombrepuntual, generaDNI};
